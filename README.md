...
- **一次性会话（开启新对话）**：每条回复提取完成并回传给网关之后，Content Script 会把页面
  **整页跳转回站点根路径**（新对话页），下一个请求即在全新会话中发起。这样可避免两类问题：
  ...
  **旧对话不会被删除**，仍保留在站点的历史记录中，只是不再被后续请求使用。
  ...
  代价是每条回复后都会整页刷新一次页面。
  **跳转可靠性（0.4.0）**：
  (1) 无条件跳转——不按 URL 判断是否已在新对话页（SPA 更新 URL 存在时序延迟，按 URL 判断可能漏重置）；
  (2) 跳转前短暂等待，确保本条结果消息已回传网关；
  (3) 上一轮跳转若未成功发起，下一条请求发送前会再尝试一次，且失败会显式告警；
  (4) 因跳转会重建 Content Script，后台在下发任务前增加了「等待目标标签页加载完成」的保护，
      避免下一条请求在刷新期间注入失败。
...
- **会话切换依赖整页跳转**：每条回复后页面会跳转回站点根路径（新对话页）并刷新，因此
  标签页会短暂处于加载状态；扩展已在下发任务前等待加载完成，但仍会带来一次可见的页面刷新。
  另外该方式默认「新对话页 = 站点根路径」，若目标站点的新对话页不在根路径（如 `/new`），
  需调整 `content.js` 中 `newChatUrl()` 的拼装规则。
