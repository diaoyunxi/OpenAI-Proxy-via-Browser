...
- **一次性会话（会话重置）**：每条回复提取完成并回传给网关之后，Content Script 会按
  「会话项菜单按钮 → 删除菜单项 → 确认删除按钮」三步删除当前会话，删除后页面自动回到新对话页，
  下一个请求即在全新会话中发起。这样可避免两类问题：调用方（CLI / Agent）自己维护的上下文
  与网页侧残留的多轮上下文互相冲突；页面上残留的旧回复干扰 DOM 差异探测导致内容提取错位。
  三步的选择器可在弹窗里按站点配置，留空时使用内置默认值（DeepSeek 网页版）。
  清理失败只记录告警日志，不影响本次已提取到的结果。
  **会话重置可靠性增强（0.3.1）**：删除不再「点击一下就走」——
  (1) 删除前基于「当前会话 URL 的会话 id」精准定位侧边栏里真正当前的会话项，写死 `:nth-child(1)` 命中错误项时自动回退到该定位，避免并发/多会话时删错；
  (2) 三步点击整体重试（最多 3 次），应对 DeepSeek 菜单/确认框偶发弹出慢；
  (3) 删除后校验页面是否回到「新对话页」（URL 不含会话 id），未回到则重试；
  (4) 上一轮清理若失败，下一条请求发送前会再尝试清理一次，且清理失败会显式告警（不再静默带着旧上下文继续）。
...
6. 「会话项菜单按钮 / 删除菜单项 / 确认删除按钮」三个选择器同样可留空：留空时使用内置默认值
   （DeepSeek 网页版）；其它站点按「打开会话菜单 → 点删除 → 点确认」这三步路径自行填写。
...
- **会话重置依赖站点 DOM**：三步点击路径会随站点改版失效；失效时只在扩展日志里告警、不影响结果，
  需及时更新弹窗里的三个选择器。内置默认选择器原本定位侧边栏**第一个**会话项，现已对 DeepSeek 增加
  「基于当前会话 URL 精准定位当前会话项」的回退（0.3.1），即使当前会话不在侧边栏首位也能正确删除；
  其它站点仍建议在弹窗里按实际 DOM 配置三个选择器，或确认该站点侧边栏会话项 `<a>` 的 `href` 含会话 id 以启用回退。
